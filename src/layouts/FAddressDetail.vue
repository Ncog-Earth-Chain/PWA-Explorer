                    ...
                    <div class="balance center-v">
                        <h3 class="h1">
                            <span v-show="cAccount">
                                <n-e-c-token-value :value="cAccount ? cAccount.totalValue : 1" convert no-currency /> <span class="nec">NEC</span>
                            </span>
                        </h3>
                        <div v-show="cAccount" class="usd">
                            <n-e-c-token-value :value="toUSD(cAccount ? cAccount.totalValue : 1)" with-price-currency no-currency />
                        </div>
                    </div>
                </f-card>
            </div>
                    ...
                    <div class="balance center-v">
                        <h3 class="h1">
                            <span v-show="'available' in cAssets">
                                <n-e-c-token-value :value="cAssets.available" convert no-currency /> <span class="nec">NEC</span>
                            </span>
                        </h3>
                        <div v-show="'available' in cAssets" class="usd">
                            <n-e-c-token-value :value="toUSD(cAssets.available)" with-price-currency no-currency />
                        </div>
                    </div>
                </f-card>
            </div>
                ...
                <f-card>
                    ...
                    <!--
                                            <div class="row no-collapse">
                                                ...
                                                <div class="col">
                                                    <div v-show="'available' in cAssets">{{ toNEC(cAssets.available) }} NEC</div>
                                                </div>
                                            </div>
                    -->
                    <div class="row no-collapse">
                        ...
                        <div class="col">
                            <div v-show="'delegated' in cAssets"><n-e-c-token-value :value="cAssets.delegated" /></div>
                        </div>
                    </div>
                    <div class="row no-collapse">
                        <div class="col f-row-label">{{ $t('view_address_detail.pending_rewards') }}</div>
                        <div class="col">
                            <div v-show="'pending_rewards' in cAssets"><n-e-c-token-value :value="cAssets.pending_rewards" /></div>
                        </div>
                    </div>
                    <div class="row no-collapse">
                        <div class="col f-row-label">{{ $t('view_address_detail.stashed_rewards') }}</div>
                        <div class="col">
                            <div v-show="'stashed' in cAssets"><n-e-c-token-value :value="cAssets.stashed" convert /></div>
                        </div>
                    </div>
                    <div class="row no-collapse">
                        ...
                        <div class="col">
                            <div v-show="'claimed_rewards' in cAssets">
                                -
                                <!--{{ toNEC(cAssets.claimed_rewards, true) }} NEC-->
                            </div>
                        </div>
                    </div>
                    <div class="row no-collapse">
        ...
        <!--
                    ...
                    <f-card>
                        ...
                        <div class="row">
                            <div class="col">
                                <div class="num-block">
                                    <h2 class="h3">{{ $t('view_address_detail.value_in_nec') }}</h2>
                                    <div class="num"><span v-show="cAccount">{{ toNEC(cAccount ? cAccount.totalValue : 1) }}</span></div>
                                </div>
                            </div>
                            <div class="col">
                                <div class="num-block">
...
<script>
    ...
    import { WEIToNEC, NECToUSD } from "../utils/transactions.js";
...
/*
                dAssetColumns: [
                    ...
                    {
                        name: 'valueInNEC',
                        label: this.$t('view_address_detail.value_in_nec'),
                        cssClass: 'align-end',
                    },
                    {
                        name: 'valueInUSD',
                ...
                if (cAccount) {
                    ...
                    if (delegations && delegations.edges) {
                        delegations.edges.forEach((_edge) => {
                            ...
                            assets.pending_rewards += (delegation && delegation.pendingRewards ? WEIToNEC(delegation.pendingRewards.amount) : 0);
                            assets.claimed_rewards += (delegation ? WEIToNEC(delegation.claimedReward) : 0);
                        });
...
/*
            getAssetItem(_assetName, _value) {
                return {
                    asset: _assetName,
                    balance: this.toNEC(_value),
                    valueInNEC: this.toNEC(_value),
...
*/
             ...
             * @return {string}
             */
            toNEC(_value, _isNumber) {
                return _isNumber ? _value : WEIToNEC(_value);
            },
             ...
             */
            toUSD(_value) {
                return NECToUSD(WEIToNEC(_value), this.$store.state.tokenPrice);
            },
