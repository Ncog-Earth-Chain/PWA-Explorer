...
export class DeFi {
    ...
    _setTokens(_tokens) {
        ...
        this.necToken = _tokens.find((_item) => _item.symbol === 'NEC');
    ...
    */
    ...
    canTokenBeDeposited(_token) {
        return _token && _token.isActive && _token.canDeposit && _token.symbol !== 'NEC';
    }
    ...
    canTokenBeTraded(_token) {
        ...
        return _token && _token.isActive && (_token.canTrade || _token.symbol === 'NEC');
     ...
     * @return {Promise<Number>}
     */
    async fetchNECTokenPrice(_to = 'USD') {
        const data = await this.apolloClient.query({
            query: gql`
